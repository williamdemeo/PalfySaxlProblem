...
algebras---namely, those that live in a \defn{Taylor variety}, which is a variety 
...
algebras. Below $\Clo(\bA)$ denotes the clone of term
operations of $\bA$, that is, is the smallest set of
operations on $\bA$ that includes the basic operations of
$\bA$ and the projections, and that is closed under general composition.
We call the clone $\Clo(\bA)$ ``trivial'' if it is generated by the
projections alone. By a \emph{tolerance} on $A$ we mean a reflexive
...
\[
\begin{bmatrix*}[r] t(a,u) & t(a,v)\\ t(b,u)&t(b,v)\end{bmatrix*},
\]
where $t$, $a$, $b$, $u$, $v$ have the following properties:
\begin{enumerate}[(i)]
...
\begin{equation}
  \label{eq:22}
t(a,u) \mathrel{\delta} t(a,v)\quad \iff \quad t(b,u) \mathrel{\delta} t(b,v),
\end{equation}
...
$\ell$, $m$, $t$, $a$, $b$, $u$, $v$ satisfying properties (i)--(iii).
...
for all 
%% $\ell \in \{0,1,2,\dots \}$, 
%% $m \in  \{1,2,\dots \}$, $f\in \Clo_{\ell + m}(\bA)$ and 
$\ell, m \in \N$,  $t\in \Clo_{\ell + m}(\bA)$, and 
$(a, b)\in A^\ell\times A^\ell$, we have
$\ker t(a, \cdot)=\ker t(b, \cdot)$, where $\forall x\in A^\ell$,
\[
\ker t(x, \cdot) := \{(u,v)\in A^m \times A^m \mid t(x,u) = t(x,v)\}.
\]
\end{remark}
%%%% 
%%%% First edit:
...
we have %$\sansC(\alpha, \beta)$ for all $\alpha \leq (0: \beta)$. 
%%  Also, by $\sansC((0:\beta), \beta)$ we have 
\begin{equation}
...
\alpha \leq (0: \beta).
\end{equation}
The relation $(0:\beta)$ is called the \defn{centralizer} 
of $\beta$.
