...
\]
with the partition $|0,1|2,3|$, and will even resort to writing 
$\alpha = |0,1|2,3|$. 
...
Thus, when $\alpha$
has uniform blocks, we have $|X|= |x/\alpha| \nb{\alpha}$.
...
if and only if $(\forall x)(\forall y)\, |x/\alpha||y/\beta| = |X|$.
...
$y \in z/\beta$ for some $z\in x/\alpha$.  In other terms, for every $x\in X$
we have
  \begin{equation}
    \label{eq:1}
...
equivalence relations.  Then the union in~(\ref{eq:1}) is disjoint
since $\alpha \meet \beta = 0_X$. We denote this by writing
  \begin{equation}
    \label{eq:100}
