...
volume = {69},
year={2013},
note={url: \href{https://github.com/williamdemeo/Overalgebras}{github.com/williamdemeo}},
...
note = {url: \href{http://math.hawaii.edu/~ralph/Classes/619/HobbyMcKenzie-FiniteAlgebras.pdf}{math.hawaii.edu}}
}		
@article {MR3076179,
...
Assume $\bA$ is a finite algebra with $\Con \bA \cong M_n$ ($n>3$), 
...
there is a Taylor term operation in $\Clo \bA$.  %% In particular, $\bA$ is not strongly abelian.
...
Assume $V$ is a minimal set and consider the algebra $\bA^*|_V$.  This has an $M_n$
...
%% QUESTION: If A is not a G-set, can the idempotent reduct of A be a G-set?  I suppose, for an idempotent algebra, to be a G-set just means it has trivial term operations (the identity and projections).
%% So, if A is essentially unary, isn't the idempotent reduct necessarily a G-set?
%% If there were no G-sets in HS(Idem(A)), then Idem(A) has a Taylor term op (by [1] or [2]), so A has a Taylor term op and we're back in Case 1.
%% [1] Bulatov-Jeavons (2001) Proposition 4.14
%% [2] Barto-Kozik-Stanovsky Theorem 2.4. 
%% ...so, I still need to figure out how to handle this final case: when A has no Taylor term op, A is not a G-set, and HS(Idem(A)) contains a G-set.  
...
\begin{figure}
  ...
  \caption{The Wheatstone Bridge defines the relation 
    ...
    $(x,y) \in \tau(\alpha_1, \alpha_2, \beta)$ iff there 
    exist $a, b \in X$ satisfying the relations in the diagram.}
  \label{fig:rho}
\end{figure}
%\end{center}
Let $f: X\to X$ be a unary operation and suppose that $f$ is compatible with each
