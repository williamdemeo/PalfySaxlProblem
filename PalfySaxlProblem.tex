...
nontrivial example where this fact holds; i.e., we wanted to know if there was
an example where $\beta$ is of lesser height than the $\alpha_i$, so that 
$|x/\beta| \leq |y/\alpha_i| < \nb{\beta}$, so that $\beta$ does not permute
with $\alpha_1$ and $\alpha_2$, and such that we cannot use the Wheatstone
bridge to prove that $\{0_X, \alpha_1, \alpha_2, \beta, 1_X\}$ is not closed.

So, to construct $\beta$, we considered the Wheatstone bridge involving
$\alpha_1$ and $\alpha_2$ and determined the pairs that would have to end up in
$\beta$ in order to achieve $\tau\subseteq \beta$.  (So that $\tau$ does not
generate a new relation in the closure.)
